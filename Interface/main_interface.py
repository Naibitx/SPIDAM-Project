...
class MainInterface:
    ...
    def setup_ui(self):
        ...
        #Button for RT60 Graphs (trigger)
        self.rt60_button = tk.Button(self.root, text="Show RT60 Graph", command=self.display_rt60_graph)
        self.rt60_button.pack(pady=10)

        # Buttons for RT60 Nagavigation
        self.rt60_previous_button = tk.Button(self.root, text = "<--", command= self.previous_band)
        self.rt60_previous_button.pack(side= tk.LEFT,pady=10)

        self.rt60_next_button = tk.Button(self.root, text = "-->", command= self.previous_band)
        self.rt60_next_button.pack(side= tk.LEFT, pady=10)
