...
def plot_rt60_bands(file_path, bands, tk_frame):
    ...
    if bands != "all" and bands not in band_ranges:
    ...
    if bands == "all":
        for band, (low, high) in band_ranges.items():
            ...
            axis.plot(time, energy_to_db, label=f"{band.capitalize()} Band")
    else:
        # Plot one specific band
        low, high = band_ranges[bands]
        sos = scipy.signal.butter(3, [low, high], btype='band', fs=sample_rate, output='sos')
        filtered_signal = scipy.signal.sosfilt(sos, data)
        energy = np.cumsum(filtered_signal[::-1]**2)[::-1]
        energy_to_db = 10 * np.log10(energy / np.max(energy))
    ...
    axis.set_title(f"RT60 Graph - {bands.capitalize()} Band" if bands != "all" else "RT60 Graph - All Bands")
    axis.set_xlabel("Time (s)")
    axis.set_ylabel("Energy (dB)")
    axis.legend()

    # Clear the frame and embed the plot
    for widget in tk_frame.winfo_children():
        widget.destroy()
    canvas = FigureCanvasTkAgg(fig, master=tk_frame)
    canvas.get_tk_widget().pack(fill=tk.BOTH, expand=True)
